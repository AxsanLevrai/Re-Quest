...
document.getElementById('bg-file-input').addEventListener('change',async e=>{const f=e.target.files[0];if(!f)return;if(window.sb&&window.currentUser){try{const path=window.currentUser.id+'/bg/background';const{error}=await window.sb.storage.from('quest-images').upload(path,f,{upsert:true});if(!error){const{data:urlData}=window.sb.storage.from('quest-images').getPublicUrl(path);tempBg={type:'image',value:urlData.publicUrl,opacity:tempBg.opacity??82,blur:tempBg.blur??0};updateBgPreview();buildBgPresets();e.target.value='';return;}}catch(err){console.error('bg upload error:',err);}}const r=new FileReader();r.onload=ev=>{tempBg={type:'image',value:ev.target.result,opacity:tempBg.opacity??82,blur:tempBg.blur??0};updateBgPreview();buildBgPresets();};r.readAsDataURL(f);e.target.value='';});
...
async function handleFiles(files){for(const f of Array.from(files)){if(f.size>10*1024*1024){showToast('Trop lourd: '+f.name);continue;}if(window.sb&&window.currentUser){try{const safeName=f.name.replace(/[^a-zA-Z0-9._-]/g,'_');const path=window.currentUser.id+'/files/'+Date.now()+'_'+safeName;const{error}=await window.sb.storage.from('quest-images').upload(path,f,{upsert:true});if(!error){const{data:urlData}=window.sb.storage.from('quest-images').getPublicUrl(path);editFiles.push({name:f.name,size:f.size,type:f.type,url:urlData.publicUrl});renderFileList();continue;}}catch(err){console.error('file upload error:',err);}}const data=await new Promise((res,rej)=>{const r=new FileReader();r.onload=e=>res(e.target.result);r.onerror=rej;r.readAsDataURL(f);});editFiles.push({name:f.name,size:f.size,type:f.type,data});}renderFileList();}
...
function openCoverPicker(id){const inp=document.createElement('input');inp.type='file';inp.accept='image/*,image/gif';inp.onchange=async e=>{const f=e.target.files[0];if(!f)return;if(f.size>8*1024*1024){showToast('Max 8Mo');return;}const g=goals.find(g=>g.id===id);if(!g)return;if(window.sb&&window.currentUser){try{const path=`${window.currentUser.id}/covers/${id}_${Date.now()}_${f.name}`;const{error:upErr}=await window.sb.storage.from('quest-images').upload(path,f,{upsert:true});if(upErr)throw upErr;const{data:urlData}=window.sb.storage.from('quest-images').getPublicUrl(path);g.cover=urlData.publicUrl;saveGoals();renderGoals();showToast('Couverture ajoutee !');return;}catch(err){console.error('Storage upload error:',err);showToast('Erreur upload, fallback local');}}const data=await new Promise((res,rej)=>{const r=new FileReader();r.onload=ev=>res(ev.target.result);r.onerror=rej;r.readAsDataURL(f);});g.cover=data;saveGoals();renderGoals();showToast('Couverture ajoutee !');};inp.click();}
...
else { window.addEventListener('auth-ready', initApp, {once:true}); }
...
function updateHPBar(){
  ...
  const MAX_W = 806; // SVG units: bar from x=787 to x=1593
...
(function(){
  ...
  function saveAS(){
    ...
    if(!window.currentUser) return; // Don't save before auth is ready
    if(window.sb && window.currentUser) {
  ...
  // ── Init ─────────────────────────────────────────────────
  // loadAS is called from initApp via auth-ready event
      ...
      <div style="background:rgba(8,6,20,0.75);border:1px solid rgba(0,245,255,0.12);border-radius:16px;overflow:hidden;backdrop-filter:blur(12px);box-shadow:0 0 40px rgba(0,245,255,0.04)">
        ...
        <div style="padding:0 24px 24px;margin-top:-36px;position:relative">
          ...
          <div style="display:flex;gap:10px;flex-wrap:wrap">
            <button onclick="window.location.href='profile-setup.html?edit=true'" style="padding:8px 20px;background:rgba(0,245,255,0.08);border:1px solid rgba(0,245,255,0.2);border-radius:6px;color:var(--accent);cursor:pointer;font-size:13px" onmouseover="this.style.background='rgba(0,245,255,0.15)'" onmouseout="this.style.background='rgba(0,245,255,0.08)'">✏️ Modifier le profil</button>
