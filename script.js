...
async async function handleFiles(files){for(const f of Array.from(files)){if(f.size>10*1024*1024){showToast('Trop lourd: '+f.name);continue;}if(window.sb&&window.currentUser&&editingId){try{const path=`${window.currentUser.id}/files/${editingId}_${Date.now()}_${f.name}`;const{error:upErr}=await window.sb.storage.from('quest-images').upload(path,f,{upsert:true});if(upErr)throw upErr;const{data:urlData}=window.sb.storage.from('quest-images').getPublicUrl(path);editFiles.push({name:f.name,size:f.size,type:f.type,url:urlData.publicUrl});renderFileList();continue;}catch(err){console.error('Storage upload error:',err);}}const data=await new Promise((res,rej)=>{const r=new FileReader();r.onload=e=>res(e.target.result);r.onerror=rej;r.readAsDataURL(f);});editFiles.push({name:f.name,size:f.size,type:f.type,data});}renderFileList();}
